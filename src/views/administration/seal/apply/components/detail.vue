  ...
  >
    ...
    <template #footer>
      <div class="dialog-footer">
        ...
        <el-button v-if="canReject" type="danger" @click="handleReject">
          拒绝
        </el-button>
...
const canApprove = computed(() => {
  ...
  if (
    selectedFlow.value &&
    selectedFlow.value.flowList &&
    selectedFlow.value.flowList.length > 0
  ) {
  ...
  if (
    checkUids.length === 0 &&
    Array.isArray(approvalFlowData.value.flowNodes)
  ) {
    const currentNode = approvalFlowData.value.flowNodes.find(
      (node) => node.check_status === 1,
    );
...
const canRevoke = computed(() => {
  ...
  const createBy =
    currentApplyData.value?.createBy ||
    currentApplyData.value?.create_by ||
    currentApplyData.value?.userId;
  ...
  proxy.$modal
    ...
    .then(() => {
      ...
      const submitData = {
        ...
        actionId: form.id, // 当前申请的ID
        flowId: selectedFlow.value.id, // 审批流程ID
        checkName: selectedFlow.value.checkTable,
        checkFiles: "", //用章申请附件
      };
