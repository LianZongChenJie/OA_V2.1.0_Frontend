...
import { addClassify, updateClassify } from "@/api/base/contract/productClassify";
...
const edit = (row, parentName) => {
  ...
  form.value = {
    id: row.id,
    pid: row.pid,
    parentName: row.pid === 0 || row.pid === '0' ? '根节点' : (parentName || row.parentName || ''),
    title: row.title,
    desc: row.desc,
